...
class OsherSolomonSolver(RiemannSolver):
    ...
    def solve(self, Q_L, Q_R, physics, zb, normal_idx: int = 1):
        ...
        Q_S0 = Q_L.copy()
        Q_S0[..., 0] = h_S0 + zb
        Q_S0[..., normal_idx] = q_S0

        Q_S1 = Q_R.copy()
        Q_S1[..., 0] = h_S1 + zb
        Q_S1[..., normal_idx] = q_S1

        Q_13 = Q_L.copy()
        Q_13[..., 0] = h_13 + zb
        Q_13[..., normal_idx] = q_13

        Q_23 = Q_R.copy()
        Q_23[..., 0] = h_23 + zb
        Q_23[..., normal_idx] = q_23

        #Calculate fluxes
        F_0 = physics.flux(Q_L, zb, normal_idx)
        F_1 = physics.flux(Q_R, zb, normal_idx)
        F_S0 = physics.flux(Q_S0, zb, normal_idx)
        F_S1 = physics.flux(Q_S1, zb, normal_idx)
        F_13 = physics.flux(Q_13, zb, normal_idx)
        F_23 = physics.flux(Q_23, zb, normal_idx)
