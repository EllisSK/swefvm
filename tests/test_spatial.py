...
from swefvm.methods.spatial import MUSCL, FirstOrder
...
def test_muscl_uniform_field_no_reconstruction_gradient():
    ...
    Q_L, Q_R = MUSCL().reconstruct_conserved_variables(mesh)
...
def test_muscl_direction_zero_matches_default():
    mesh = Mesh1D(
        length=10.0,
        resolution=1.0,
        initial_conditions=lambda x: np.column_stack([1.0 + 0.1 * x, 0.05 * np.ones_like(x)]),
    )
    Q_L_d, Q_R_d = MUSCL().reconstruct_conserved_variables(mesh)
    Q_L_0, Q_R_0 = MUSCL().reconstruct_conserved_variables(mesh, 0)
    np.testing.assert_array_equal(Q_L_d, Q_L_0)
    np.testing.assert_array_equal(Q_R_d, Q_R_0)


def test_first_order_direction_zero_matches_default():
    mesh = Mesh1D(
        length=4.0,
        resolution=1.0,
        initial_conditions=lambda x: np.column_stack([x, x * 2]),
    )
    Q_L_d, Q_R_d = FirstOrder().reconstruct_conserved_variables(mesh)
    Q_L_0, Q_R_0 = FirstOrder().reconstruct_conserved_variables(mesh, 0)
    np.testing.assert_array_equal(Q_L_d, Q_L_0)
    np.testing.assert_array_equal(Q_R_d, Q_R_0)